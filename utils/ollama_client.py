...
_TIMEOUT    = int(os.getenv("OLLAMA_TIMEOUT",    "0"))     # 0 = no timeout (wait as long as needed for 100% accuracy)
...
# ── Scoreboard OCR prompt (High-Precision Full Guide) ──────────────────────────
_SCOREBOARD_PROMPT = """\
...
4. Read each row carefully and accurately. Never copy or duplicate numbers from adjacent rows. Each player has their own distinct ACS, K/D/A, and damage stats.
5. Average Combat Score (ACS) is the second column, K/D/A is the third column, Damage is the fourth column. Ensure values match the columns accurately.
6. Use null for any number you cannot read confidently. Never guess.
7. Return ONLY the JSON object. Nothing before or after.
"""
...
def _prepare_image(image_bytes: bytes, max_dim: int = 1600) -> bytes:
    """Resize huge screenshots to max 1600px to maintain crisp OCR accuracy while fitting within context limits."""
    try:
...
async def _call_ollama_internal(image_bytes: bytes, prompt: str, json_format: bool = False) -> str:
    ...
    image_bytes = _prepare_image(image_bytes, max_dim=1600)
